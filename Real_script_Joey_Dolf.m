...
mfurate = 0.23;                                  % [kg/cyc]  Fuel per otto cycle 
...
Qloss = HeatLoss(Ca(i), T(i-1), p(i-1), pr, Tr, V(i-1), k(i), p1, V1, Vr);
Q = -dQcom(i)* dCa + Qloss*(dCa)/(360*50);
dT=(Q-p(i-1)*dV)/Cv_mix(i)/m(i-1); % 1st Law dU=dQ-pdV (closed system)
...
if pr == 0
    ...
    hc      = 3.26*B^(-0.2)*p^(0.8)*T^(-0.55)*omega^(0.8);
    h       = (V - Vc)/(Vd)*S;
    A       = pi/2*B^2 + 2*pi*B*h;
    if T < Tw
        Qloss = 0;
